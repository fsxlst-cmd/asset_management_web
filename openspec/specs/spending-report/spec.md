...
Defines a monthly spending report that aggregates expense entries by category for a single WIB calendar month, ranks categories by spend with each category's share of the month total, compares each category against the previous month, surfaces categories that went quiet, and buckets unresolvable categories under "Uncategorised" without dropping them.
